...
import { ShareIcon, HeartIcon, ClipboardIcon } from '@heroicons/react/24/outline';
                ...
                return (
                    <div key={`${zikr.id}-${idx}`} className="bg-white dark:bg-[#1A3129] rounded-xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm transition-colors duration-300">
                        <div className="flex justify-between items-start mb-3">
                            ...
                            <button onClick={() => toggleFavorite(zikr.id)}>
                        ...
                        <div className="flex justify-between items-center text-gray-500 dark:text-gray-500 text-xs mt-4">
                            ...
                            <div className="flex space-x-4 rtl:space-x-reverse text-gray-400 dark:text-gray-500">
                                <button className="hover:text-primary-500"><ShareIcon className="w-4 h-4" /></button>
                                <button className="hover:text-primary-500"><ClipboardIcon className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                );
            })}
            <div className="text-center text-gray-500 text-sm py-4">End of stream</div>
        </div>
    );
};
