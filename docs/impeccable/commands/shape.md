...
Reference for the `/impeccable shape` command.
---## When to use it
...
Note: if you want the discovery and the build in one go, just describe the surface. A plain /impeccable a settings screen for team billing runs the same discovery, resolves the visual direction with you, and continues into implementation. See New work. shape standalone is for when you want only the brief, so you can take it to whatever implementation approach you prefer.
